...
def get_img_fname(base_fname, base_path, extension, z_index, z_rng):
    if z_index >= z_rng[1]:
    ...
    for m in matches:
        if m:
            ...
            z_str = str(z_index).zfill(int(m.split(':')[1]))
        else:
            z_str = str(z_index)
...
def read_img_stack(img_size, datatype, z_slices, base_fname, base_path, extension, s3_res, s3_bucket_name, z_rng, warn_missing_files=False):
    send_msg('{} Reading image data (z range: {}:{})\n'.format(
    ...
    for idx, z_slice in enumerate(z_slices):
        img_fname = get_img_fname(base_fname, base_path, extension, z_slice, z_rng)
    ...
    send_msg('{} Finished reading image data\n'.format(get_formatted_datetime()))
    return im_array
...
def assert_equal(rmt, z_rng, channel_resource, resolution, img_size, datatype, base_fname, base_path, extension, s3_res, s3_bucket_name):
    ...
    try:
        send_msg('{} Downloading full size z-slice from BOSS\n'.format(get_formatted_datetime()))
...
def main():
    ...
    parser.add_argument('datasource', type=str, help='Location of files, either "local" or "s3"')
